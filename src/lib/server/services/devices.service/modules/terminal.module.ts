...
export function isDeviceTerminalEligible(input: {
	userRoles: string[];
	device: { platform: string; adoptionMode: string; adoptionState: string };
...
export default (_ctx: ServiceModuleContext) => ({

	isEligible: isDeviceTerminalEligible,
	startSession: startDeviceTerminalSession
});
