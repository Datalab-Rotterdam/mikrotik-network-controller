...
} from '$lib/server/services/alerts.service';
    ...
    try {
        ...
        if (bridgePortResult.status === 'fulfilled') {
            for (const bp of bridgePortResult.value) {
                const ifaceName = str(bp['interface'] ?? bp.interface);
                if (!ifaceName) continue;
                bridgePortMap.set(ifaceName, {
                    pvid: num(bp['pvid'] ?? bp.pvid),
        ...
        if (ifaceResult.status === 'fulfilled') {
            ...
            // Update deviceInterfaces with running state + bridge port VLAN data
            const interfaceUpdates = ifaceResult.value
                .filter((iface) => str(iface['name'] ?? iface.name))
                .map((iface) => {
                    ...
                    return {
                        ...
                        name,
                        type: str(iface['type'] ?? iface.type) ?? undefined,
                        macAddress: str(iface['mac-address'] ?? iface.macAddress) ?? undefined,
                        running: iface['running'] === 'true' || iface['running'] === true,
...
function registerInterval(callback: () => void, delay: number): void {
    intervals.add(setInterval(callback, delay));
}
...
export function startMonitoring(): void {
    ...
    startupTimer = setTimeout(() => {
        ...
        void pollAll();
        registerInterval(() => void pollAll(), POLL_INTERVAL_MS);

        // Prune metrics older than 30 days, once per day
        void MetricsRepository.pruneOld(30);
        registerInterval(() => void MetricsRepository.pruneOld(30), 24 * 60 * 60 * 1_000);
        ...
        async function backupAllDevices() {
            try {
                ...
                for (const d of managed) {
                    Service('scheduler')
                        .schedule(backupDeviceTask(d.id, d.siteId ?? undefined))
                        .catch(() => {
                        });
                }
            } catch {
        ...
        registerInterval(() => void backupAllDevices(), 24 * 60 * 60 * 1_000);
            ...
            try {
                ...
                for (const d of routeros) {
                    void Service('scheduler')
                        .schedule(checkFirmwareTask(d.id, d.siteId ?? undefined))
                        .catch(() => {
                        });
                }
            } catch {
        ...
        registerInterval(() => void checkAllFirmware(), 24 * 60 * 60 * 1_000);
    }, STARTUP_DELAY_MS);
}
...
const monitoringEvents = new AnyEventEmitter<MonitoringEventMap>();
...
export const service = {
    ...
    dependsOn: ['scheduler'],
    load: () => {
        monitoringEvents.any(async (event, detail) => {
