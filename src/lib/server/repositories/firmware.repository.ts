...
export async function upsertFirmwareVersion(input: {
	deviceId: string;
...
export async function getFirmwareVersion(deviceId: string) {
	const [row] = await db
		.select()
...
export async function getFirmwareVersionsForDevices(deviceIds: string[]) {
