<script lang="ts">
	...
	function deviceKind(platform: string | null | undefined, model: string | null | undefined): 'router' | 'switch' {
	...
	const topologyDevices = $derived<TopologyDevice[]>([
		...data.devices.map((device) => ({
			...
			interfaceName: '',
			interfaces: data.interfaces.filter((networkInterface) => networkInterface.deviceId === device.id),
			connection: null
		})),
	...
	const rootDevice = $derived(topologyDevices.find((device) => device.adopted) ?? topologyDevices[0]);

	function buildNodes(): Node<DeviceNodeData>[] {
		const adoptedDevices = topologyDevices.filter((device) => device.adopted);
		const discoveredDevices = topologyDevices.filter((device) => !device.adopted);
	...
	function buildEdges(): Edge[] {
		...
		return topologyDevices
			.filter((device) => device.nodeId !== rootDevice.nodeId)
			.map((device) => ({
				id: `${rootDevice.nodeId}-${device.nodeId}`,
				source: rootDevice.nodeId,
				target: device.nodeId,
				type: 'smoothstep',
				label: device.interfaceName || (device.adopted ? 'inventory' : 'discovered'),
				style: device.adopted ? undefined : 'stroke-dasharray: 6 5;'
			}));
	}

	let nodes = $state<Node<DeviceNodeData>[]>(buildNodes());
	let edges = $state<Edge[]>(buildEdges());
	let selectedNodeId = $state('');
	const selectedDevice = $derived(topologyDevices.find((device) => device.nodeId === selectedNodeId));
	const selectedConnection = $derived(edges.find((edge) => edge.source === selectedNodeId || edge.target === selectedNodeId));
	const adoptedCount = $derived(data.devices.length);
	const discoveredCount = $derived(data.discoveredDevices.length);
...
<section class="topology-page" class:with-panel={Boolean(selectedDevice)}>
	...
	<section class="topology-shell">
		...
		{:else}
			<div class="empty-state">Topology appears after devices are discovered or adopted.</div>
		{/if}
	</section>
		...
		>
			<div class="topology-details">
				...
				<div class="details-card">
					...
					<div class="info-row">
						<span>Source</span>
						<strong>{selectedDevice.adopted ? 'Inventory' : 'MNDP discovery'}</strong>
					</div>
...
<style lang="scss">
	...
	.topology-shell :global(.svelte-flow__node.selected .device-node) {
		border-color: #0f6fff;
		box-shadow: 0 0 0 3px rgba(15, 111, 255, 0.12), 0 8px 18px rgba(14, 14, 16, 0.08);
	}
