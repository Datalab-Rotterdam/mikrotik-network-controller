...
export const load = enhance.load(
	async ({ params }) => {
		const [site, sites, unacknowledgedAlertCount, deviceCount] = await Promise.all([
			SiteRepository.findById(params.site_id),
			SiteRepository.list(),
			AlertRepository.countUnacknowledged(params.site_id),
			DeviceRepository.countBySite(params.site_id)
		]);
		...
		return { site, sites, unacknowledgedAlertCount, deviceCount };
	},
