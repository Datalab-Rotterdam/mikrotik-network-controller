<script lang="ts">
	...
	const adoptionPanelOpen = $derived(data.adoptionPanel.open || Boolean(form?.message));
	...
	const panelApiPort = $derived(form?.apiPort ?? (panelPlatform === 'switchos' ? 80 : 8728));
	...
	const selectedDevice = $derived(rows.find((device) => device.id === selectedDeviceId));
	const detailsPanelOpen = $derived(Boolean(selectedDevice) && !adoptionPanelOpen);
...
<section class="devices-page" class:with-panel={anyPanelOpen}>
	<div class="devices-toolbar">
		...
		<a class="icon-button" href={`${basePath}/devices?adopt=`} aria-label="Adopt device" title="Adopt device">
	...
	>
		<Form action="?/adopt">
				...
				<Input label="Username" name="username" autocomplete="username" value={form?.username ?? 'admin'} required />
				<Input label="Password" name="password" type="password" autocomplete="current-password" required />

				<details class="advanced-settings">
					<summary>Advanced settings</summary>
					<div class="advanced-fields">
						<Input label="Host" name="host" placeholder="192.168.88.1" value={panelHost} required />
						<Input label="API port" name="apiPort" inputmode="numeric" value={panelApiPort} required />
						<Input label="Site" name="siteName" value={panelSiteName} required />
						<label class="field">
							<span>Device OS</span>
							<select name="platform">
								<option value="routeros" selected={panelPlatform !== 'switchos'}>RouterOS</option>
								<option value="switchos" selected={panelPlatform === 'switchos'}>SwitchOS</option>
							</select>
						</label>
						<label class="field">
							<span>Provider</span>
							<select name="provider">
								<option value="real" selected={panelProvider !== 'mock'}>Real RouterOS API</option>
								<option value="mock" selected={panelProvider === 'mock'}>Mock device</option>
							</select>
						</label>
					</div>
				</details>
		...
		>
			<div class="device-details">
				<div class="details-tabs" aria-label="Device sections">
					<span class="active">
						<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
							<path fill="currentColor" d="M7 3h2v18H7V3Zm8 0h2v18h-2V3ZM3 8h2v8H3V8Zm16 0h2v8h-2V8Z" />
						</svg>
						Overview
					</span>
					<span>
						<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
							<path fill="currentColor" d="M5 19h14v2H5v-2Zm1-8h3v6H6v-6Zm5-8h3v14h-3V3Zm5 5h3v9h-3V8Z" />
						</svg>
						Insights
					</span>
					<span>
						<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
							<path fill="currentColor" d="m19.4 13.5.1-1.5-.1-1.5 2-1.5-2-3.5-2.4 1a8.8 8.8 0 0 0-2.6-1.5L14 2h-4l-.4 2.5A8.8 8.8 0 0 0 7 6L4.6 5 2.6 8.5l2 1.5-.1 1.5.1 1.5-2 1.5 2 3.5 2.4-1a8.8 8.8 0 0 0 2.6 1.5L10 22h4l.4-2.5A8.8 8.8 0 0 0 17 18l2.4 1 2-3.5-2-1.5ZM12 15.5A3.5 3.5 0 1 1 12 8a3.5 3.5 0 0 1 0 7.5Z" />
						</svg>
						Settings
					</span>
				</div>
				...
				<div class="details-card">
					...
					<div class="info-row">
						<span>Status</span>
						<strong>{selectedDevice.adopted ? selectedDevice.status : 'Discovered'}</strong>
					</div>
					<div class="info-row">
					...
					{#if selectedDevice.adopted}
						...
						<div class="info-row">
							<span>Uptime</span>
							<strong>{formatUptime(selectedDevice.details.uptimeSeconds)}</strong>
						</div>
						<div class="info-row">
							<span>Last Sync</span>
								...
								<div class="interface-block">
									<div class="interface-title">
										...
										<span class:online={networkInterface.running}>
											{networkInterface.disabled ? 'Disabled' : networkInterface.running ? 'Running' : 'Inactive'}
										</span>
									</div>
									<div class="info-row">
										<span>Type</span>
