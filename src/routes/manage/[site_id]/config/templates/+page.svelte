<script lang="ts">
  ...
  let deployDryRunResult = $state<{ rendered: string; diff: string } | null>(
    null,
  );
  ...
  const deployableDevices = $derived(
    devices.filter(
      (d: Device) =>
        d.platform === "routeros" && d.adoptionState === "fully_managed",
    ),
  );
  ...
  <div class="editor-panel">
      ...
      >
        <label>
          ...
          <input
            ...
            bind:value={editName}
            required
            placeholder="e.g. Default OSPF config"
          />
        </label>
        <label>
        ...
        <label class="content-label">
          <span
            >Content <small class="hint"
              >(use &#123;&#123;variable&#125;&#125; placeholders)</small
            ></span
          >
          <textarea
            name="content"
            ...
            spellcheck="false"
            placeholder="# Paste RouterOS export here…"
          ></textarea>
        </label>
    ...
    {:else if selected && showDeploy}
      ...
      >
        ...
        <input
          type="hidden"
          name="variableValues"
          value={JSON.stringify(deployVariableValues)}
        />
        ...
        <div class="deploy-actions">
          <Button
            ...
            onclick={() => {
              ...
              fetch(".", {
                method: "POST",
                body: fd,
                headers: { "X-Form-Action": "/dryRun" },
              })
                .then((r) => r.json())
                .then((j) => {
                  if (j.success) {
                    deployDryRunResult = {
                      rendered: j.renderedContent,
                      diff: j.diff,
                    };
                  }
                })
          ...
          <Button type="submit" variant="primary" disabled={!deployDeviceId}>
      ...
      <form method="POST" action="?/update" use:enhance class="edit-form">
        ...
        <input
          type="hidden"
          name="variables"
          value={JSON.stringify(selected.variables ?? [])}
        />
        <label>
          <span>Name</span>
        ...
        {#if (selected.variables ?? []).length > 0}
          <div class="variables-section">
            ...
            <p class="var-hint">
              Variable metadata is auto-extracted from placeholders. Edit the
              JSON in the API or re-save to refresh.
            </p>
          </div>
        {/if}
    ...
    {:else}
      <div class="empty-panel">
        ...
        <Button variant="secondary" onclick={startCreate} type="button"
          >+ New template</Button
        >
      </div>
    {/if}
